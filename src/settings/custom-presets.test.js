...
const presetJson = (name, scale = ['100.', '1200.'], equivSteps = 2) => JSON.stringify({
  name,
  scale,
  equivSteps,
});
...
describe('CustomPresets import actions', () => {
  ...
  it('keeps new files and skips clashes when overwrite is declined', async () => {
    localStorage.setItem('hexatone_custom_presets', JSON.stringify([
      { name: 'Existing clash', scale: ['2/1'], equivSteps: 1 },
    ]));
...
describe('CustomPresets save, export and delete', () => {
  ...
  it('deletes the selected preset and calls onClear', () => {
    localStorage.setItem('hexatone_custom_presets', JSON.stringify([
      { name: 'Delete Me', scale: ['2/1'], equivSteps: 1 },
    ]));
    const onClear = vi.fn();
