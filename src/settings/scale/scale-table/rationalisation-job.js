...
export function startRationalisationJob(input, onProgress = () => {}) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    const finish = (error, scale) => {
      ...
      settled = true;
      worker?.terminate();
      if (error) reject(error); else resolve(scale);
    };
    if (typeof Worker === "undefined") {
      // Compatibility fallback for environments without workers.
      Promise.resolve().then(() => {
        if (settled) return;
        try { finish(null, rationaliseScaleBatch(input, onProgress)); }
        catch (error) { finish(error); }
      });
      return;
    }
    try {
      worker = new Worker(new URL("./batch-rationalise.worker.js", import.meta.url), { type: "module" });
      ...
      worker.onerror = event => finish(new Error(event.message || "Rationalisation worker failed"));
