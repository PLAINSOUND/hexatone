...
const TuneCell = ({
  ...
  const preview = getDegreePreview(previewState, degree);
  const tunedCents = preview?.cents ?? null;
  const comparing = isDegreeComparing(previewState, degree);
  const originalCents = getCommittedDegreeCents(workspace, degree);
  ...
  }, [scaleStr, committedCents, preview]);
  ...
  useEffect(() => {
    ...
    onPreviewChangeRef.current?.((prevState) => clearDegreePreview(prevState, degree));
    setPreviewInterval(null);
    setRationaliseCandidates(null);
  }, [resetVersion, degree]);
  ...
  const currentCents = tunedCents !== null ? tunedCents : originalCents;
  const isDirty = tunedCents !== null && Math.abs(tunedCents - originalCents) > 0.001;
  ...
  const onPointerMove = useCallback(
    (e) => {
      ...
      onPreviewChangeRef.current?.((prevState) =>
        setDegreeComparing(setDegreePreview(prevState, degree, newCents), degree, false),
      );
  ...
  const onCompare = useCallback(() => {
    ...
    onPreviewChangeRef.current?.((prevState) => setDegreeComparing(prevState, degree, next));
    glideTo(next ? originalCents : tunedCents);
  }, [comparing, degree, glideTo, originalCents, tunedCents]);

  const onSave = useCallback(() => {
    const saveVal = tunedCents !== null ? tunedCents : originalCents;
    ...
    if (degree === 0) {
      ...
      const delta = tunedCents - originalCents;
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  onPreviewChangeRef.current?.((prevState) =>
                    setDegreeComparing(
                      setDegreePreview(prevState, degree, candidate.cents),
                      degree,
                      false,
                    ),
                  );
