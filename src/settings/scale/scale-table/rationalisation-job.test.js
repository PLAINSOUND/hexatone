...
const input = {
  settings: { scale: ["5/4", "3/2", "2/1"], fundamental: 440, reference_degree: 0 },
  searchPrefs: { existingRatios: "keep" },
  frequencies: [440, 550, 660, 880],
  committedCents: [0, 386.3137, 701.955, 1200],
};
...
it("searches cents-valued degrees while preserving the equave", () => {
  const result = rationaliseScaleBatch({
    ...input,
    settings: { ...input.settings, scale: ["386.3137", "701.955", "2/1"] },
  });
  expect(result).toEqual(["5/4", "3/2", "2/1"]);
});
