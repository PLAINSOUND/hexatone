...
const MidiTuning = (props) => (
  <>
    ...
    <label>
      ...
      <select
        value={String(props.settings.sysex_type)}
        name="sysex_type"
        onChange={(e) => {
    ...
    <p>
      <em>
        The <a href="/midituning.html">MIDI Tuning Standard</a>, described in detail at{" "}
        <a href="https://midi.org/midi-tuning-updated-specification">midi.org</a>, allows external
        synthesizers to receive data modifying the tuning of each MIDI note. This is done by system
        exclusive messages: either a non-real-time "Bulk Tuning Dump" or 128 real-time "Single-Note
        Tuning Changes". The receiving synth will need to be set to receive sysex into the specified
        Tuning Map slot. Using the free{" "}
        <a href="https://oddsound.com/mtsespmini.php">Oddsound MTS-ESP Mini</a> plug-in, it is
        possible to translate MTS data to retune softsynths using other protocols (MPE or
        multichannel pitchbend).
      </em>
    </p>
  </>
);
