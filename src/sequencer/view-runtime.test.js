...
describe("sequencer view runtime", () => {
  it("finds the first snapshot id present in a set", () => {
    expect(
      firstSnapshotIdInSet(new Set(["s2", "s3"]), [{ id: "s1" }, { id: "s2" }, { id: "s3" }]),
    ).toBe("s2");
  });
  ...
  it("builds cue-expanded snapshot ids at a cue index", () => {
    ...
    const sequenceEvents = [
      ...
      { type: "note", cueIndex: 1, kind: "attack", snapshotId: "s2", eventId: "s2:b:attack:0.25" },
      {
        ...
        kind: "release",
        snapshotId: "s1",
        eventId: "s1:a:release:1.25",
      },
    ];
    expect(buildCueExpandedSnapshotIdsAt(0, snapshots, bars, tempi, sequenceEvents)).toEqual(
      new Set(["s1", "s2"]),
    );
  });
  ...
  it("derives the complete cue viewport plan in the rendered event-id domain", () => {
    ...
    expect(
      ...
      cueIndex: 8,
      soundingAttackEventIds: new Set(["rendered:arrival-a:on", "rendered:arrival-b:on"]),
      eventIds: ["rendered:held:off", "rendered:arrival-a:on", "rendered:arrival-b:on"],
  ...
  it("resolves cue anchor snapshots with the same precedence used by app and autoscroll", () => {
    ...
    expect(
      resolveCueAnchorSnapshotId({
        activeCueIndex: 13,
        sequenceCueGroups: [{ snapshotIndex: 0 }, { snapshotIndex: 1 }, { snapshotIndex: 2 }],
        sequenceEvents: [{ type: "note", cueIndex: 13, snapshotId: "s3", eventId: "s3:c13" }],
        snapshots: [{ id: "s1" }, { id: "s2" }, { id: "s3" }],
        cueExpandedSnapshotIds: new Set(),
      }),
    ).toBe("s3");
  ...
  it("includes every Flight Cue 9 attack across the early and later snapshot groups", () => {
    ...
    const cueNineAttacksInDisplaySnapshotNine = events.filter(
      (event) =>
        event.type === "note" &&
        event.kind === "attack" &&
        event.cueIndex === 9 &&
    ...
    expect(
      cueNineAttacksInDisplaySnapshotNine.every((event) => soundingIds.has(event.eventId)),
    ).toBe(true);
    expect(
      buildCueExpandedSnapshotIdsAt(
        8,
