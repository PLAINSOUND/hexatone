...
import {
  deriveSnapshotDegreeList,
  deriveSnapshotFilterEntries,
} from "./snapshot-filter-library.js";
...
describe("snapshot-filter-library", () => {
  ...
  it("maps snapshot notes onto reduced degrees of the current tuning", () => {
    expect(
      deriveSnapshotDegreeList([{ midicents: 69 }, { midicents: 64 }, { midicents: 60 }], runtime),
    ).toEqual([0, 4, 9]);
  });
    ...
    expect(
      deriveSnapshotFilterEntries(
        [
          ...
          {
            id: 2,
            notes: [{ midicents: 67 }, { midicents: 62 }],
          },
        ],
        runtime,
      ),
  ...
  it("derives snapshot degrees from raw Scala-style scale text", () => {
    expect(
      deriveSnapshotDegreeList([{ midicents: 69 }, { midicents: 64 }, { midicents: 60 }], {
        scale: [
          "100.0",
          "200.0",
          "300.0",
  ...
  it("matches FALL snapshot 1 against the built-in Sabat The Tree tuning by exact degree identity", () => {
    ...
    const sequence = JSON.parse(
      readFileSync("src/sequencer/preset-sequences/marc-sabat/FALL.json", "utf8"),
    );
    ...
    expect(deriveSnapshotDegreeList(sequence.snapshots[0].notes, runtime)).toEqual([
      11, 17, 29, 34, 56,
    ]);
  });
