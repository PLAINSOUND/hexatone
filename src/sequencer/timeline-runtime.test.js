...
describe("sequencer timeline runtime", () => {
  ...
  it("derives first note events per snapshot and their starting cue indexes", () => {
    ...
    expect(firstSnapshotEventIds).toEqual(
      new Map([
        ["s1", "note:1"],
        ["s2", "note:2"],
      ]),
    ...
    expect(buildSnapshotStartCueIndexes(firstSnapshotEventIds, sequenceEvents)).toEqual(
      new Map([
        ["s1", 4],
        ["s2", 7],
      ]),
  ...
  it("builds first-event and first-cue indexes for transport navigation", () => {
    ...
    expect(buildFirstCueIndexBySnapshotIndex(cueGroups)).toEqual(
      new Map([
        [0, 0],
        [2, 2],
      ]),
    );
    expect(buildFirstCueTimeBySnapshotIndex(cueGroups)).toEqual(
      new Map([
        [0, 1.125],
        [2, 3.25],
      ]),
  ...
  it("tracks the first event per snapshot per cue for courtesy markers", () => {
    ...
    expect(buildFirstSnapshotCueEventIds(snapshotEventsById)).toEqual(
      new Map([
        ["s1:1", "s1:e1"],
        ["s1:2", "s1:e3"],
        ["s2:2", "s2:e1"],
  ...
  it("expands snapshots relevant to the active cue and still shows sustained attacks", () => {
    const ids = buildCueExpandedSnapshotIds(
      5,
      [
        {
          type: "note",
  ...
  it("derives the selected cue absolute time from either the selected marker or the playhead cue", () => {
    ...
    expect(deriveSelectedCueAbsoluteTime(null, 1, cueGroups, snapshotIndexById)).toBe(2.5);
  });
});
