...
export const roundTowardZero = (val) => (val < 0 ? Math.ceil(val) : Math.floor(val));
...
export const hexCoordsToCents = (coords, settings) => {
  ...
  if (reducedSteps_prev < 0) {
    reducedSteps_prev += len;
    octs_prev -= 1;
  }
  if (reducedSteps_next < 0) {
    reducedSteps_next += len;
    octs_next -= 1;
  }
 ...
 *
 * @param {object} settings - { midiin_anchor_note, midiin_anchor_channel, equivSteps, rSteps, drSteps, gcd }
...
export const midiNoteToCoords = (settings, noteNumber, midiChannel) => {
  const { midiin_anchor_note, midiin_anchor_channel, equivSteps, rSteps, drSteps, gcd } = settings;
  ...
  return new Point(rSteps_count + gcdSteps_count * gcd[1], drSteps_count + gcdSteps_count * gcd[2]);
};
