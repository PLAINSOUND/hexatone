...
const MIDI_INPUT_REQUIRED_EVENT_NAMES = ["noteon", "noteoff", "controlchange", "pitchbend"];
...
function safePortField(port, field) {
  try {
    return port?.[field] ?? null;
  } catch {
    return null;
  }
}
...
function inputHasListener(input, eventName) {
  if (!input || typeof input.hasListener !== "function") return false;
  try {
...
function hasRequiredMidiInputListeners(input) {
  return MIDI_INPUT_REQUIRED_EVENT_NAMES.every((eventName) => inputHasListener(input, eventName));
}

function isHakenMpePlusInputActive() {
  return (
    (this.controller?.id === "hakenaudio" ||
      this.settings?.midiin_controller_override === "hakenaudio") &&
...
function linnstrumentUfGlideCurve(deviation, shapeSetting) {
  ...
  const curved = Math.pow(Math.max(0, Math.min(1, transition)), Math.max(0.25, transitionShape));
...
function linnstrumentUfRowNeighborCents(channel, col, fallbackCents) {
  ...
  const lookupCents = (targetCol, direction) => {
    ...
    if (currentCoords) {
      const [cents] = this.hexCoordsToCents(
        new Point(currentCoords.x + direction, currentCoords.y),
      );
      return cents;
    }
...
function linnstrumentUfXFilterThresholds(
  pressure = 127,
  reductionSetting = LINNSTRUMENT_UF_SPIKE_REDUCTION_DEFAULT,
) {
  ...
  const blend =
    strength > LINNSTRUMENT_UF_SPIKE_REDUCTION_DEFAULT
      ...
      : (LINNSTRUMENT_UF_SPIKE_REDUCTION_DEFAULT - strength) /
        LINNSTRUMENT_UF_SPIKE_REDUCTION_DEFAULT;
...
function linnstrumentUfNormalizeX14(
  msb,
  lsb,
  reductionSetting = LINNSTRUMENT_UF_SPIKE_REDUCTION_DEFAULT,
) {
...
function filterLinnstrumentUfX14(key, x14, pressure = 127) {
  ...
  if (state.pending != null && Math.abs(x14 - state.pending) <= confirmTolerance) {
    state.pending = x14;
...
function smoothLinnstrumentUfX14(key, x14, pressure = 127) {
  const strength = Math.max(
    ...
    Math.min(
      100,
      Number(this.settings.linnstrument_x_input_smoothing) ||
        LINNSTRUMENT_UF_X_INPUT_SMOOTHING_DEFAULT,
    ),
  );
  if (strength === 0) {
...
function linnstrumentUfOnsetAssist(key, shapeSetting) {
  ...
  const rampMs =
    LINNSTRUMENT_UF_ONSET_RAMP_BASE_MS +
...
export function rebindMidiInput(options = {}) {
  const syncColors = options.syncColors !== false;
  recordMidiRestoreDiagnostic({
    type: "input-rebind",
    detail: safePortField(this.midiin_data, "id") ?? safePortField(this.settings, "midiin_device"),
  });
  teardownMidiInput.call(this);
  setupMidiInput.call(this);
  if (syncColors) syncControllerAutoColors.call(this);
}

export function ensureMidiInputBinding(options = {}) {
  const force = options?.force === true;
  const selectedId = this.settings?.midiin_device;
  if (!selectedId || selectedId === "OFF") return false;
  recordMidiRestoreDiagnostic({
    type: "input-ensure",
  ...
  const selectedChanged = selectedInput !== this.midiin_data;
  const missingListeners = !hasRequiredMidiInputListeners(selectedInput);
  if (!force && !selectedChanged && !missingListeners) return true;
  recordMidiRestoreDiagnostic({
    type: "input-ensure-rebind",
    detail: force ? "force" : selectedChanged ? "selected-changed" : "missing-listeners",
  });
  rebindMidiInput.call(this, { syncColors: false });
  return hasRequiredMidiInputListeners(this.midiin_data);
}
            ...
            if (cc >= 65 && cc <= 89 && linnstrumentUfInputActive) {
              ...
              if (this.inputRuntime.mpeInput) {
                const hasHakenMpePlusLsb =
                  hakenMpePlusInputActive && this._hakenMpePlusLsbByChannel.has(e.message.channel);
                ...
                this._mpeInputCC74ByChannel.set(e.message.channel, {
                  value,
                  value14,
                  time: Date.now(),
                });
      ...
      this.midiin_data.addListener("channelaftertouch", (e) => {
        withMidiJitterInput(
          ...
          () => {
            ...
            if (this.inputRuntime.mpeInput) {
              if (value14 != null)
                this._hakenMpePressure14ByChannel.set(e.message.channel, value14);
              this._mpeInputAftertouchByChannel.set(e.message.channel, {
                value,
                value14,
                time: Date.now(),
              });
